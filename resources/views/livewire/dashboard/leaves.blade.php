...
<div>
                            ...
                            <div class="table-responsive p-0">
                                ...
                                <table class="table align-items-center mb-0">
                                    ...
                                    <tbody>
                                        ...
                                        @else
                                            @foreach ($dataLeave as $index => $leave)
                                                <tr>
                                                    <td class="p-4">{{ $dataLeave->firstItem() + $index }}</td>
                                                    <td class="p-1">
                                                    ...
                                                    <td class="p-1 align-middle">
                                                        <h6
                                                            class="badge badge-sm @if ($leave->status == 'approved') bg-gradient-success @elseif ($leave->status == 'rejected') bg-gradient-danger @elseif ($leave->status == 'cancelled') bg-gradient-primary @else bg-gradient-secondary @endif  p-2 text-uppercase">
                                                            {{ $leave->status }}</h6>
                                                    </td>
                                                    <td class="p-0">
                                                        ...
                                                        <button wire:click="showModalLeave({{ $leave->id }})"
                                                            type="button"
                                                            class="badge badge-sm bg-gradient-info text-white border-0"
                                                            style="color: inherit" data-bs-toggle="modal"
                                                            data-bs-target="#showLeaveModal">
                                                            <i class="fa-solid fa-eye"></i>
                                                        </button>
                                                        <button wire:click="showModalReject({{ $leave->id }})"
                                                            type="button"
                                                            class="badge badge-sm bg-gradient-danger text-white border-0"
                                                            style="color: inherit" data-bs-toggle="modal"
    ...
    @include('livewire.dashboard.include.reject-leave-form-modal')
    @include('livewire.dashboard.include.show-data-leave-modal')
</div>
@push('additional-js')
